...
#include "XOR_4BIT.h"
    ...
    void mono_pulse(sc_signal<bool> &pulse){
        ...
        sc_start(5, SC_NS);
        pulse.write(0);
        sc_start(5, SC_NS);  // Add this to make sure signal is consumed
    };
#endif
...
int sc_main(int argc, char* argv[]) {
    ...
    sc_signal<bool> en("en");                 // Enable signal
    sc_signal<sc_uint<4>> a("a");
    sc_signal<sc_uint<4>> b("b");
    sc_signal<sc_uint<4>> out("out");

    // Instantiate XOR_4BIT
    XOR_4BIT xor_mod("xor_mod");
    xor_mod.RST(rst);
    xor_mod.EN(en);
    xor_mod.A(a);
    xor_mod.B(b);
