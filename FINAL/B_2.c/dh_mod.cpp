...
/// ================================================ ///
#ifndef sigWrite
#define sigWrite(x, val) ((x).write(val))
#endif
#ifndef sigRead
#define sigRead(x) ((x).read())
#endif
#ifndef SC_TRACE
...
// Declare Signals as Global So That They can be used
// in the "extern" function to interface with the C
// code
sc_signal<bool> enable_s("enable_s"), done_s("done_s");
sc_signal<sc_uint<32>> dataI1("dataI1"), dataI2("dataI2"), dataO1("dataO1"), 
						dataO2("dataO2");

// Extern Function allows C code to recognize
// SystemC code
extern "C" void NN_DigitMult_SC(NN_DIGIT a[2], NN_DIGIT b, NN_DIGIT c)
{
	...
	do {
		wait(1, SC_NS);
	} while (!sigRead(done_s));

	// Read result
	a[0] = sigRead(dataO1); // low
	a[1] = sigRead(dataO2); // high
...
// Copy & Paste your 64-bit multiplier code in here
// Modify your model so that it will run concurrently
// with the other components of this design. Make
// sure your model uses a some type of handshake protocol.
SC_MODULE(nnDIGITmult)
{
	sc_in<bool> enable;
	...
	sc_signal<EXE_STATE_TYPE> exe_state;

	SC_CTOR(nnDIGITmult):
	enable("enable"), in_data_1("in_data_1"), in_data_2("in_data_2"), 
	out_data_low("out_data_low"), out_data_high("out_data_high"), done("done")
	{
		SC_THREAD(process);
	}
	...
	void process()
	{
		...
		{
			wait(1, SC_NS);
			if (enable.read() == 1 && exe_state.read() == WAIT)
			{
				exe_state.write(EXECUTE);
			}

			if (exe_state.read() == EXECUTE)
			{
				sc_uint<64> result = (sc_uint<64>)in_data_1.read() * (sc_uint<64>)in_data_2.read();
				out_data_low.write(result.range(31, 0));
				out_data_high.write(result.range(63, 32));
				done.write(1);
				exe_state.write(OUTPUT);
			}
			else if (exe_state.read() == OUTPUT)
			{
				if (enable.read() == 0)
				{
					done.write(0);
					exe_state.write(WAIT);
				}
			}
		}
	}
};
// Copy and paste your 64-bit multiplication code above
...
SC_MODULE(dhdemo){
	...
	*/
	...
	/*static*/ void DoSetupAgreement(R_RANDOM_STRUCT *randomStruct, int priv)
	{
		...
		*/
		do
		{
			if (status = R_SetupDHAgreement(publicValue, privateValue, privateValueLen, params, randomStruct))
			{
	...
	/*static*/ void DoComputeAgreedKey()
	{
		...
		*/
		do
		{
			...
			if (status = R_ComputeDHAgreedKey(agreedKey, otherPublicValue, privateValue, privateValueLen, params))
			{
	...
	/*static*/ void DoGenerateParams(R_RANDOM_STRUCT *randomStruct)
	{
		...
		if (status = R_GenerateDHParams(&PARAMS2, primeBits, subPrimeBits, randomStruct))
		{
	...
	*/
	...
	static void WriteParams2()
	{
		...
		while (1)
		{
			GetCommand(filename, sizeof(filename),
	...
	static int GetParams(R_DH_PARAMS **params, char *prompt)
	{
		...
		while (1)
		{
			...
			case '2':
				if (!PARAMS2_READY)
				{
					PrintError("ERROR: Parameters 2 have not been generated yet.  Try Again.", 0);
					break;
				}
				else
				{
...
int sc_main(int argc, char* argv[])
{
	...
	nnDIGITmult mult("mult");
	mult.enable(enable_s);
	mult.in_data_1(dataI1);
	mult.in_data_2(dataI2);
	mult.done(done_s);
	mult.out_data_low(dataO1);
	mult.out_data_high(dataO2);
