...
#include "digit.h"
    ...
    extern "C"
    {
        void NN_DigitMult_SC(NN_DIGIT a[2], NN_DIGIT b, NN_DIGIT c);
    }
#else
    void NN_DigitMult_SC(NN_DIGIT a[2], NN_DIGIT b, NN_DIGIT c);
#endif
...
{
    NN_DigitMult_SC(a, b, c);
    // Call the "Extern" Function to link up with the hardware
    // implementation instead of doing the operation in software
}
